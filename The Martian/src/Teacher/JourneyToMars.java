...
public class JourneyToMars extends JFrame implements ActionListener, ChangeListener, FocusListener, Runnable {
	...
	public void paint(Graphics g) {
		...
		semimajorAxis_toMarsGA.setText(String.format("%.3f",  mission.getEarth2MarsGA().getSemimajorAxis()));
		...
		eccentricity_toMarsGA.setText(String.format("%.3f",  mission.getEarth2MarsGA().getEccentricity()));		
	}
    ...
    public void stateChanged(ChangeEvent event) {
		if(event.getSource() == semimajorAxisSlider_toEarth) {
            ...
            //}			
		} else if(event.getSource() == semimajorAxisSlider_toMarsGA) {
            //if(!semimajorAxisSlider_toMarsGA.getValueIsAdjusting())
            //{
    			int value = semimajorAxisSlider_toMarsGA.getValue();
    			double a = Mission.aMin_toEarth + (Mission.aMax_toEarth-Mission.aMin_toEarth) / (sliderIncrement) * value;
    			mission.getEarth2MarsGA().setSemimajorAxis(a);				
    			//adjust eccentricity accordingly
    			mission.getEarth2MarsGA().setEccentricity(Mission.rMars/a-1.0);	
            //}
		}
	...
	public void actionPerformed(ActionEvent event) {	
		...
		} else if(event.getSource() == semimajorAxis_toMarsGA) {
			System.out.println("semimajorAxis_toMarsGA");
			double a_toEarth = Double.parseDouble(semimajorAxis_toMarsGA.getText());
			if(a_toEarth > Mission.aMax_toEarth)
				a_toEarth = Mission.aMax_toEarth;
			else if(a_toEarth < Mission.aMin_toEarth)
				a_toEarth = Mission.aMin_toEarth;				
			mission.getEarth2MarsGA().setSemimajorAxis(a_toEarth);			
			//adjust eccentricity accordingly
			mission.getEarth2MarsGA().setEccentricity(Mission.rMars/a_toEarth-1.0);
		...
		} else if(event.getSource() == eccentricity_toMarsGA) {
			double e_toEarth = Double.parseDouble(eccentricity_toMarsGA.getText());
			if(e_toEarth > Mission.eMax_toEarth)
				e_toEarth = Mission.eMax_toEarth;
			else if(e_toEarth < Mission.eMin)
				e_toEarth = Mission.eMin;
			mission.getEarth2MarsGA().setEccentricity(e_toEarth);				
			//adjust semimajor accordingly
			mission.getEarth2MarsGA().setSemimajorAxis(Mission.rMars/(1+e_toEarth));	
		...
		} else if(event.getSource() == showEarth2MarsGA) {
			if(event.getActionCommand().equals("show")) {
				showEarth2MarsGA.setActionCommand("hide");
				showEarth2MarsGA.setText("Hide GA");
				mission.setShowEarth2MarsGA(true);
			} else {
				showEarth2MarsGA.setActionCommand("show");				
				showEarth2MarsGA.setText("Show GA");
				mission.setShowEarth2MarsGA(false);
			}
	...
	public static void main(String [] args) {
    	...
    	Orbit earth2MarsGA = new Orbit("Earth to Mars (Gravity Assist)", Mission.aMax_toEarth, Mission.eMin);
    	earth2MarsGA.setOmega(180.0); //return trip
    	earth2MarsGA.setDestinationOrbit(earth);
    	
		//launch the app with a new mission
		j2m.launch(new Mission(earth, mars, earth2Mars, mars2Earth, earth2MarsGA));
	}
	...
	private void initialize() {
		...
		controls1.setBorder(UIManager.getBorder("MenuBar.border"));
		FlowLayout flowLayout = (FlowLayout) controls1.getLayout();
		flowLayout.setAlignment(FlowLayout.LEFT);
		bottom.add(controls1);
		...
		controls1.add(daysLabel);
		
		dayField = new JTextField();
		dayField.setColumns(5);
		controls1.add(dayField);
		dayField.addActionListener(this);
		dayField.addFocusListener(this);
		
		addDay = new JButton("+");
		controls1.add(addDay);		
		addDay.addActionListener(this);
		
		minusDay = new JButton("-");
		controls1.add(minusDay);
		minusDay.addActionListener(this);
		
		showEarth2Mars = new JButton("Show Departure");
		controls1.add(showEarth2Mars);
		showEarth2Mars.addActionListener(this);
		showEarth2Mars.setActionCommand("show");		

		showMars2Earth = new JButton("Show Return");
		controls1.add(showMars2Earth);
		showMars2Earth.addActionListener(this);
		showMars2Earth.setActionCommand("show");	

		showEarth2MarsGA = new JButton("Show GA");
		controls1.add(showEarth2MarsGA);
		showEarth2MarsGA.addActionListener(this);
		showEarth2MarsGA.setActionCommand("show");		

		JPanel controls2 = new JPanel();
		controls2.setBorder(UIManager.getBorder("MenuBar.border"));
		flowLayout = (FlowLayout) controls2.getLayout();
		flowLayout.setAlignment(FlowLayout.LEFT);
		bottom.add(controls2);		
		
		showEarth2MarsRetrograde = new JButton("Show Retrograde");
		controls2.add(showEarth2MarsRetrograde);
		...
		toMarsGA = new JPanel();
		toMarsGA.setBorder(UIManager.getBorder("MenuBar.border"));
		bottom.add(toMarsGA);
		toMarsGA.setLayout(new FlowLayout(FlowLayout.LEFT, 5, 5));
		
		JLabel toMarsGALabel = new JLabel("Earth To Mars (GA):");
		toMarsGA.add(toMarsGALabel);
		
		JLabel semimajorAxisLabel_toMarsGA = new JLabel("a (AU):");
		toMarsGA.add(semimajorAxisLabel_toMarsGA);
		
		semimajorAxis_toMarsGA = new JTextField();
		semimajorAxis_toMarsGA.setColumns(10);
		toMarsGA.add(semimajorAxis_toMarsGA);
		semimajorAxis_toMarsGA.addActionListener(this);
	
		semimajorAxisSlider_toMarsGA = new JSlider(0, sliderIncrement, sliderIncrement);
		semimajorAxisSlider_toMarsGA.setPreferredSize(new Dimension(100, 23));
		toMarsGA.add(semimajorAxisSlider_toMarsGA);
		semimajorAxisSlider_toMarsGA.addChangeListener(this);
		
		JLabel eccentricityLabel_toMarsGA = new JLabel("e:");
		toMarsGA.add(eccentricityLabel_toMarsGA);
		
		eccentricity_toMarsGA = new JTextField();
		eccentricity_toMarsGA.setColumns(10);
		toMarsGA.add(eccentricity_toMarsGA);
		eccentricity_toMarsGA.addActionListener(this);		
		
		JPanel playback = new JPanel();
		playback.setBorder(UIManager.getBorder("MenuBar.border"));
		flowLayout = (FlowLayout) playback.getLayout();
		flowLayout.setAlignment(FlowLayout.LEFT);
		bottom.add(playback);
