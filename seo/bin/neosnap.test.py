...
while True:
    ...
    if(altaz >= min_elevation):
        slackdebug('Pointing telescope to %s (RA=%s, DEC=%s, AZ=%s, ALT=%s)...'%(primary_name, primary_ra, primary_dec, altaz.alt.degree, altaz.az.degree))
        (output, error, pid) = runSubprocess(['tx','point','%s'%primary_ra, '%s'%primary_dec])
        ...
        for primary_filter in primary_filters:
            ...
            (output, error, pid) = runSubprocess(['image','dark','time=%d'%primary_t_exposure,'bin=%d'%primary_bin, 'outfile=%s'%fits])    
            if not error:
    ...
    if float(ch['EL'][0]) >= min_elevation:
        ...
        (output, error, pid) = runSubprocess(['tx','point','%s'%ra, '%s'%dec])
