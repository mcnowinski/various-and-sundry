...
re_doFilter = '^done filter num=([0-9]+) name=([\S]+)'

def calcFilters():
    for filter in filters:
       print 'The %s filter was used %d times.'%(filter[0], len(filter[1]))
...
def calcWeather():
    ...
    y = [(100-cloud) for (date, cloud, rain) in weather]
    fig = plt.figure()
    ...
    plt.xlabel('Day')
<<<<<<< HEAD:seo/itzamna/unlogger.windows.py
    plt.ylabel('Sunshine-ee-ness (%)')
    plt.ylim([0,100])
...
>>>>>>> 7b8233ab4b1ea04bf2b42d0de591f18382b1854f:seo/itzamna/unlogger.py
...
def doFilter(msg, dt):
    match = re.search(re_doFilter, msg)
    if match:
...
[re_doWhere, doWhere],
#done filter num=3 name=r-band
[re_doFilter, doFilter],
...
<<<<<<< HEAD:seo/itzamna/unlogger.windows.py
    dtFilterStart = parser.parse(os.sys.argv[1])
    dtFilterEnd = parser.parse(os.sys.argv[2])
    filterDates = True
