...
class query():
    ...
    def get_ephemerides(self, observatory_code,
        ...
        #doc 072017 added moon phase and illumination
        quantities = '1,3,4,8,9,10,18,19,20,21,23,24,25,27,31,33,36'
        ...
        for line in datablock:
            ...
            for idx,item in enumerate(headerline):
                ...
                if (item.find('Illu%') == 0):
                    ...
                    datatypes.append(np.float64)
                    datatypes.append(np.float64)
                #doc 072017 moon phase angle
                if (item.find('T-O-M') > -1):
                    try:
                        this_eph.append(np.float64(line[idx]))
                        #print(line[idx].strip())
                    except ValueError:
                        this_eph.append(np.nan)
                    fieldnames.append('tom')
                    datatypes.append(np.float64)
                #doc 072017 moon illumination
                if (item.find('MN_Illu%') > -1):
                    try:
                        this_eph.append(np.float64(line[idx]))
                        #print(line[idx].strip())
                    except ValueError:
                        this_eph.append(np.nan)
                    fieldnames.append('moonillum')
                    datatypes.append(np.float64)
