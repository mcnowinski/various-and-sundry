...
public class DS9Batch {
	
	//main
    public static void main(String[] args) {
    	
    	boolean doRename = false; //rename input files to ###.fits, helps to avoid long command line errors 
    	
    	for(int i=0; i<args.length; i++) {
    		if(args[i].equals("-rename")) {
    			doRename = true;
    			System.out.println("Rename is set to ON.");
    		}
    	...
    	//get runtime dir
		String runtimePath = DS9Batch.class.getResource(DS9Batch.class.getSimpleName() + ".class").getFile();
		if(runtimePath.startsWith("/")) {
			runtimePath = runtimePath.substring(1, runtimePath.lastIndexOf('/'));
			runtimePath = runtimePath.replaceAll("%20"," ");
		} else { //file:/
			runtimePath = runtimePath.substring(6, runtimePath.lastIndexOf('!'));
			runtimePath = runtimePath.substring(0, runtimePath.lastIndexOf('/'));			
			runtimePath = runtimePath.replaceAll("%20"," ");
		}
    	String ds9Command = runtimePath + "/ds9.exe";
    	...
    	String ds9Batch = runtimePath + "/ds9.bat";		
		
    	//allow user to choose folder containing asteroid images
		String path;
		JFileChooser chooser = new JFileChooser();
	    chooser.setCurrentDirectory(new java.io.File("."));
	    chooser.setDialogTitle("Select a folder containing your asteroid images.");
    	...
    	try {
	    	...
	    	for (File tempFile : files) {
	    		File inputFile;
	    		if(doRename) {	  			
	    			inputFile = new File(path + "/" + String.format("%03d", count) + ".fits");
	    		...
	    		ds9Parameters += " \"" + inputFile.getPath() + "\""; 
	    	} 
			...
			try {
				Process proc = Runtime.getRuntime().exec(new String [] {ds9Batch});
