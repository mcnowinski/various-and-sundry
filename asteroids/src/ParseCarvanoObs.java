                ...
                if(!id.equals(lastId)) {
                	if(lastId.length() > 0) { //not first hit
		        		...
		        		outputFileW.write(lastId + "," + countObs + "," + taxClass + "," + totalClass + "," + finalTaxClass + "," + finalTaxClassNumber + "," + totalCount + "," + finalTaxClassCountUnique  + "," +
		        				totalScore + "," + String.format("%.2f", totalCount > 0 ? totalScore/totalCount : 0.0) + "," + (Double.isNaN(stddev_single) ? "" : stddev_single) + "," + String.format("%.2f", (Double.isNaN(mean_class) ? 0.0 : mean_class)) + "," + (Double.isNaN(stddev_class) ? "" : stddev_class) + "," +
                	...
                	if(taxClass.length() > 2 || taxClass.length() == 0) {
                    	...
                    	taxScore = String.format("%.2f", Double.parseDouble(fields[6])/100.0);
