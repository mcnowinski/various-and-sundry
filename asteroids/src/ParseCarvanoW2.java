...
public class ParseCarvanoW2 {	
    ...
    public static void main(String[] args) {        
        try { 
            ...
            outputFileW.write("AST_NAME_PROV_DESIG,OBS_COUNT,OBS_CLASSES,TOTAL_CLASSES,CLASS,CLASS_NUM,CLASS_COUNT,TOTAL_COUNT,TOTAL_SCORE,AMBIGUITY,AVE_SCORE,STDEV,KURTOSIS,C,X,D,L,A,S,Q,O,V,Cw,Xw,Dw,Lw,Aw,Sw,Qw,Ow,Vw\n");
                ...
                if(!id.equals(lastId)) {
                	if(lastId.length() > 0) { //not first hit
                		...
                		System.out.println("	Bad C/X/D/L/A/S/Q/O/V = " +
                				...
                				badL + "/" + badA + "/" + badS + "/" +
                				badQ + "/" + badO + "/" + badV);
                		outputFileW.write(lastId + "," + countObs + "," + taxClass + "," + totalClass + "," + finalTaxClass + "," + finalTaxClassNumber + "," + totalCountUnique + "," +
                				totalCount + "," + totalScore + "," + String.format("%.2f", kurtosis > 0 ? 1.0/kurtosis : 0.0) + "," + String.format("%.2f", totalCount > 0 ? totalScore/totalCount : 0.0) + "," + (Double.isNaN(stdev) ? "" : stdev) + "," + (Double.isNaN(kurtosis) ? "" : kurtosis) + "," +
