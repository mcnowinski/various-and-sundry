...
for new in sorted(im):
    ...
    if(solve_field == True):
        ...
        output = subprocess.check_output(solve_field_path + ' --no-fits2fits --overwrite --downsample 2 --scale-units arcsecperpix --scale-low 0.55 --scale-high 2.00  --cpulimit 30 --no-plots %s'%(new), shell=True)
        log.write(output)
        #print output
