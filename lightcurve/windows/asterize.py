...
if(solve_field == True):
    ...
    try:
        ...
        ra = raA
        dec = decA
    solve_field_command = solve_field_path + ' --no-fits2fits --overwrite --downsample 2 --crpix-center --scale-low %f --scale-high %f --scale-units arcsecperpix --ra %s --dec %s --radius %f --cpulimit 30 --no-plots ' % (
        scale_low, scale_high, ra, dec, radius) + '"%s"' % (os.path.abspath(input_fits).replace('\\', '/'))
    output = subprocess.check_output(
        '%s -l -c "%s"' % (cygwin_path, solve_field_command), shell=True)
else:
...
if(remove_comment_history == True):
    ...
    d1[0].writeto('%s.newer' % (input_fits.rsplit('.', 1)[
        0]), overwrite=True, output_verify='exception')
d1.close()
os.remove('%s.new' % (input_fits.rsplit('.', 1)[0]))
try:
...
os.system('mv %s.newer %s' % (input_fits.rsplit('.', 1)[0], output_file))
