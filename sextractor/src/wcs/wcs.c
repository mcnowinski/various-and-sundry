...
*   Summary of routines
*   -------------------
*   wcsfwd() and wcs_rev() are high level driver routines for the WCS linear
...
*   unknown elements.
*
*   An initialization routine, wcs_set(), computes indices from the ctype
*   array but need not be called explicitly - see the explanation of
*   wcs.flag below.
*
*
*   Initialization routine; wcs_set()
*   --------------------------------
...
*   it will be invoked by wcsfwd() and wcs_rev() if the "flag" structure member
...
*
*
*   Reverse transformation; wcs_rev()
*   --------------------------------
...
*             0 to 5 as above.
*
*       These routines support both methods; wcs_set() determines which is
*       being used by the presence or absence of a CUBEFACE axis in ctype[].
*       wcsfwd() and wcs_rev() translate the CUBEFACE axis representation to
...
*         wcs_set() to redetermine the indices.  The flag is set to 999 if
...

{
   ...
   if (wcs->flag != WCSSET) {
      if (wcs_set(lin->naxis, ctype, wcs)) return 1;
   }
...
int wcs_rev(ctype, wcs, pixcrd, lin, imgcrd, prj, phi, theta, crval, cel,
    world)
