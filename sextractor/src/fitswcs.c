...
/* Test if the WCS is recognized and a celestial pair is found */
  wcs_set(wcs->naxis,(const char(*)[9])wcs->ctype, wcs->wcsprm);
...
/* Test if the WCS is recognized and a celestial pair is found */
  if (!wcs_set(wcs->naxis,(const char(*)[9])wcs->ctype, wcs->wcsprm)
	&& wcs->wcsprm->flag<999)
    {
     char	*pstr;
     double	date;
...
int	raw_to_wcs(wcsstruct *wcs, double *pixpos, double *wcspos)
  ...
  if (wcs_rev((const char(*)[9])wcs->ctype, wcs->wcsprm, pixpos,
...
/* Initialize if required */
  if (wcsprm && wcsprm->flag != WCSSET)
    {
    if (wcs_set(wcs->naxis, (const char(*)[9])wcs->ctype, wcsprm))
      return RETURN_ERROR;
    }
